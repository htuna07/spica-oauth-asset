...
const APIKEY = process.env.OAUTH_APIKEY;
...
export async function upsertGoogle(req, res) {
  ...
  if (!strategy) {
    ...
    return axios
      .post(`${PUBLIC_URL}/passport/strategy`, strategy, {
        headers: { Authorization: `APIKEY ${APIKEY}` },
      })
      .then((r) =>
        res.send({
          message: `Successfully completed. Please add this '${r.data.options.code.params.redirect_uri}' url to the 'Authorized redirect URIs' from Google Cloud Platform.`,
        })
      );
  }
  ...
  return axios
    .put(`${PUBLIC_URL}/passport/strategy/${strategy._id}`, strategy, {
      headers: { Authorization: `APIKEY ${APIKEY}` },
    })
    .then((r) =>
      res.send({
        message: `Changes have been saved. Please make sure that you have already add this '${r.data.options.code.params.redirect_uri}' url to the 'Authorized redirect URIs' from Google Cloud Platform.`,
      })
    );
}
...
export async function upsertFacebook(req, res) {
  ...
  if (!strategy) {
    ...
    return axios
      ...
      .then((r) =>
        res.send({
          message: `Successfully completed. Please add this '${r.data.options.code.params.redirect_uri}' url to the 'Valid OAuth Redirect URIs' from Facebook Developers.`,
        })
      );
  }
  ...
  return axios
    ...
    .then((r) =>
      res.send({
        message: `Changes have been saved. Please make sure that you have already add this '${r.data.options.code.params.redirect_uri}' url to the 'Valid OAuth Redirect URIs' from Facebook Developers.`,
      })
    );
}
...
async function findStrategy(name) {
  const { data: strategies } = await axios.get(
    `${PUBLIC_URL}/passport/strategies`
  );
  ...
  return axios
    .get(`${PUBLIC_URL}/passport/strategy/${strategy._id}`, {
      headers: { Authorization: `APIKEY ${APIKEY}` },
    })
    .then((r) => r.data);
}
